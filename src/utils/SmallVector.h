...
protected:
  ...
  void grow_pod(void *FirstEl, size_t MinCapacity, size_t TSize) {
    ...
    void *NewElts;
    if (BeginX == FirstEl) {
      NewElts = detail::safe_malloc(NewCapacity * TSize);

      // Copy the elements over.  No need to run dtors on PODs.
      memcpy(NewElts, this->BeginX, size() * TSize);
    } else {
      // If this wasn't grown from the inline copy, grow the allocated space.
      NewElts = detail::safe_realloc(this->BeginX, NewCapacity * TSize);
    }
...
template <typename T, typename... Ts> struct AlignedCharArrayUnion {
  alignas(detail::AlignerImpl<T, Ts...>) char buffer[sizeof(detail::SizerImpl<T, Ts...>)];
};
...
void SmallVectorTemplateBase<T, TriviallyCopyable>::grow(size_t MinSize) {
  ...
  T *NewElts = static_cast<T*>(detail::safe_malloc(NewCapacity*sizeof(T)));
...
public:
  ...
  iterator insert(iterator I, size_type NumToInsert, const T &Elt) {
    ...
    if (size_t(this->end()-I) >= NumToInsert) {
      ...
      std::fill_n(I, NumToInsert, Elt_copy);
      return I;
    }
    ...
    std::fill_n(I, NumOverwritten, Elt_copy);

    // Insert the non-overwritten middle part.
    std::uninitialized_fill_n(OldEnd, NumToInsert-NumOverwritten, Elt_copy);
    return I;
  }
